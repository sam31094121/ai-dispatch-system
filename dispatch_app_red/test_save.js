...
`;
const req = http.request('http://localhost:3000/api/save', {
  method: 'POST',
  headers: { 'Content-Type': 'application/json' }
}, res => {
  let body = '';
  res.on('data', d => body += d);
  res.on('end', () => console.log(JSON.parse(body).data?.consistencyGuard));
});
req.write(JSON.stringify({ operator: 'WEB', rawText: text }));
req.end();
