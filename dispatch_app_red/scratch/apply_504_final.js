...
const sourceText = `📣【AI 派單公告｜5/4 結算 → 5/5 正式派單順序｜AI 比例原則版】
...
async function apply504Final() {
    console.log('--- 兆櫃 AI 專業更新：5/4 業績全面同步 ---');
    
    // 1. 執行解析
    const report = buildReportFromSource({
        sourceText,
        reportId: 'dispatch_2026_05_04',
        status: 'published'
    });

    // 2. 手動注入「名次異動」數據 (prevRank)
    const movementData = {};
    const movementLines = sourceText.split('\n').filter(l => l.includes(' → '));
    movementLines.forEach(line => {
        const match = line.match(/^([^：: \t]+)[:：]?\s*(\d+)\s*→/u);
        if (match) {
            movementData[match[1]] = Number(match[2]);
        }
    });

    report.rankings.forEach(row => {
        if (movementData[row.name]) {
            row.prevRank = movementData[row.name];
            if (row.rank < row.prevRank) row.movement = 'up';
            else if (row.rank > row.prevRank) row.movement = 'down';
            else row.movement = 'flat';
        }
    });
    ...
    report.aiProvider = {
        mode: 'ELITE_BIG_DATA',
        engine: 'Professional AI Boss v9',
        optimizedAt: new Date().toISOString()
    };
    report.persisted = true;

    // 4. 儲存
    const stored = saveReportVersion(report, {
        operator: 'AI_PROFESSIONAL_BOSS',
        reason: '5/4 業績全面優化更新，包含名次異動與比例原則。',
        source: 'manual-elite-input'
    });

    console.log(`[SUCCESS] 5/4 業績已完成優化導入！`);
    console.log(`[VERSION] 目前系統版本：v${stored.report.version}`);
    console.log(`[AUDIT] 審計結果：${stored.report.auditResult}`);
    console.log(`[SYNC] 名次異動數據已對齊。`);
}

apply504Final().catch(console.error);
