...
for (const line of lines) {
  if (line.match(/^[\u4e00-\u9fa5]+(\（.*\）)?$/) && !line.includes('產品') && !line.includes('三立') && !line.includes('日期') && !line.includes('行銷')) {
...
// Insert 陳旭宜 at the end just in case, or ignore. Usually ignored in ranking list.
const groups = {
...
const newObjString = `const OFFICIAL_0428_TO_0429 = Object.freeze({
  ...
  overallStats: {
    totalCalls: 650, 
    dispatchCalls: 400, 
    renewalCalls: 368, // (255+75+38)
    renewalAmount: 4637068, // (3161848+468330+1006890)
    dailyRenewalAmount: 512000, 
    cancellations: 0,
    monthlyRevenue: 8177760, // (4456550+2679320+1041890)
    actualRevenue: 6104042 // (3725432+1834480+544130)
  },
  platforms: {
    '三立奕心': {
...
// Insert it right after OFFICIAL_0427_TO_0428
const searchStr = 'const OFFICIAL_0427_TO_0428 = Object.freeze({';
const startIdx = content.indexOf(searchStr);

if (startIdx > -1) {
  // Find the end of the OFFICIAL_0427_TO_0428 block
  const nextFunctionIdx = content.indexOf('function official0427GroupOf(name) {');
  
  if (nextFunctionIdx > -1) {
      const newContent = content.substring(0, nextFunctionIdx) + newObjString + '\\n\\n' + content.substring(nextFunctionIdx);
  ...
  console.log('Could not find OFFICIAL_0427_TO_0428');
}
