...
function parseData() {
  ...
  const ranking = rankingLines.map((line) => {
    ...
    let advice = "";
    const advLine = adviceLines.find(al => al.includes(name) && al.includes('：'));
  ...
  const snapshot = {
    ...
    adviceList: ranking.map(r => ({ name: r.name, rank: r.rank, group: r.group, text: r.advice })),
    finalConfirmations: [
        "5/4 結算資料已核對完成",
        "3平台總表全數核對通過",
        "無漏算、無多算、無總盤衝突。",
  ...
  return {
    ...
    meta: {
      ...
      reason: '5/4 -> 5/5 Final Optimization'
    }
  };
}
