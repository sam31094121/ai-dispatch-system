module.exports = {
  "report": {
    "reportId": "dispatch_2026_05_04_v1",
    "version": 1,
    "status": "published",
    "title": "AI 派單公告｜5/4 結算 → 5/5 正式派單",
    "settlementDate": "2026-05-04",
    "dispatchDate": "2026-05-05",
    "reportDate": "115/05/04",
    "auditResult": "PASS",
    "createdAt": "2026-05-04T19:30:00+08:00",
    "updatedAt": "2026-05-04T11:27:45.671Z",
    "sourceText": "📣【AI 派單公告｜5/4 結算 → 5/5 正式派單順序｜AI 比例原則版】\n\n一、審計結論\n\n審計結果：PASS\n本輪已依鎖死規則執行：先審計，後運算，後排序，再派單。\n\n三立奕心總表核對通過\n追續單成交：45 ✅\n全部總業績：717,358 ✅\n追續單金額：575,440 ✅\n實收總金額：60,460 ✅\n\n民視產品總表核對通過\n追續單成交：12 ✅\n全部總業績：490,000 ✅\n追續單金額：80,590 ✅\n實收總金額：18,160 ✅\n\n公司產品總表核對通過\n追續單成交：4 ✅\n全部總業績：45,710 ✅\n追續單金額：43,230 ✅\n實收總金額：35,250 ✅\n\n異常與提醒\n本輪三平台總表與個別明細加總一致。\n無漏算、無多算、無總盤衝突。\n\n二、整合總盤\n\n【追續單成交】61\n【全部總業績】1,253,068\n【追續單金額】699,260\n【實收總金額】113,870\n\n三、AI 比例原則\n\n本輪權重：\n實收總業績：3000\n追續單金額：2500\n全部總金額：1500\n追續客單價：1500\n追續單數量：1500\n\n正式權重分數＝\n實收比例分數＋追續金額比例分數＋全部總業績比例分數＋追續客單價比例分數＋追續單數量比例分數\n\n本輪全員最高值：\n實收總金額最高：30,480（湯玉琦）\n追續單金額最高：189,510（馬秋香）\n全部總業績最高：244,150（馬秋香）\n追續客單價最高：27,072.86（馬秋香）\n追續單數量最高：7（王珍珠、馬秋香、林沛昕）\n\n四、正式名次\n\n1、馬秋香｜【正式權重分數】7000.00｜【實收】0｜【追續金額】189,510｜【全部總業績】244,150｜【追續客單價】27,072.86｜【追續單數】7\n2、湯玉琦｜【正式權重分數】6253.76｜【實收】30,480｜【追續金額】62,860｜【全部總業績】106,860｜【追續客單價】12,572.00｜【追續單數】5\n3、林沛昕｜【正式權重分數】6073.93｜【實收】18,100｜【追續金額】95,580｜【全部總業績】126,148｜【追續客單價】13,654.29｜【追續單數】7\n4、廖姿惠｜【正式權重分數】3661.65｜【實收】2,980｜【追續金額】70,660｜【全部總業績】97,710｜【追續客單價】17,665.00｜【追續單數】4\n5、王珍珠｜【正式權重分數】3188.03｜【實收】0｜【追續金額】42,650｜【全部總業績】128,230｜【追續客單價】6,092.86｜【追續單數】7\n6、林宜靜｜【正式權重分數】3077.17｜【實收】7,600｜【追續金額】31,380｜【全部總業績】112,760｜【追續客單價】10,460.00｜【追續單數】3\n7、高美雲｜【正式權重分數】2930.50｜【實收】11,250｜【追續金額】26,070｜【全部總業績】42,490｜【追續客單價】6,517.50｜【追續單數】4\n8、周美蓁｜【正式權重分數】2292.29｜【實收】12,000｜【追續金額】12,000｜【全部總業績】12,000｜【追續客單價】12,000.00｜【追續單數】1\n9、許喬恩｜【正式權重分數】2292.29｜【實收】12,000｜【追續金額】12,000｜【全部總業績】12,000｜【追續客單價】12,000.00｜【追續單數】1\n10、莉莉（新人）｜【正式權重分數】2271.51｜【實收】11,880｜【追續金額】11,880｜【全部總業績】11,880｜【追續客單價】11,880.00｜【追續單數】1\n11、徐華妤｜【正式權重分數】2105.03｜【實收】0｜【追續金額】35,640｜【全部總業績】35,640｜【追續客單價】17,820.00｜【追續單數】2\n12、高如郁｜【正式權重分數】2039.76｜【實收】7,580｜【追續金額】15,500｜【全部總業績】37,640｜【追續客單價】7,750.00｜【追續單數】2\n13、李玲玲｜【正式權重分數】1842.52｜【實收】0｜【追續金額】18,560｜【全部總業績】78,690｜【追續客單價】4,640.00｜【追續單數】4\n14、王梅慧｜【正式權重分數】1742.81｜【實收】0｜【追續金額】20,800｜【全部總業績】52,600｜【追續客單價】5,200.00｜【追續單數】4\n15、梁依萍｜【正式權重分數】1486.55｜【實收】0｜【追續金額】14,280｜【全部總業績】47,640｜【追續客單價】14,280.00｜【追續單數】1\n16、林佩君｜【正式權重分數】1312.99｜【實收】0｜【追續金額】14,700｜【全部總業績】14,700｜【追續客單價】14,700.00｜【追續單數】1\n17、江麗勉｜【正式權重分數】898.95｜【實收】0｜【追續金額】9,480｜【全部總業績】13,460｜【追續客單價】4,740.00｜【追續單數】2\n18、陳百玲（新人）｜【正式權重分數】756.90｜【實收】0｜【追續金額】6,980｜【全部總業績】6,980｜【追續客單價】3,490.00｜【追續單數】2\n19、鄭珮恩｜【正式權重分數】752.80｜【實收】0｜【追續金額】5,750｜【全部總業績】14,500｜【追續客單價】2,875.00｜【追續單數】2\n20、謝啟芳｜【正式權重分數】518.85｜【實收】0｜【追續金額】2,980｜【全部總業績】16,300｜【追續客單價】2,980.00｜【追續單數】1\n21、陳玲華｜【正式權重分數】158.14｜【實收】0｜【追續金額】0｜【全部總業績】25,740｜【追續客單價】0｜【追續單數】0\n22、江沛林｜【正式權重分數】91.85｜【實收】0｜【追續金額】0｜【全部總業績】14,950｜【追續客單價】0｜【追續單數】0\n23、蘇淑玲｜【正式權重分數】0｜【實收】0｜【追續金額】0｜【全部總業績】0｜【追續客單價】0｜【追續單數】0\n24、鄭上官｜【正式權重分數】0｜【實收】0｜【追續金額】0｜【全部總業績】0｜【追續客單價】0｜【追續單數】0\n\n五、名次異動\n\n以上一輪 5/3 正式派單名次對照，本輪異動如下：\n\n上升\n馬秋香：2 → 1 ↑\n林沛昕：4 → 3 ↑\n廖姿惠：9 → 4 ↑\n高美雲：13 → 7 ↑\n徐華妤：21 → 11 ↑\n梁依萍：18 → 15 ↑\n林佩君：22 → 16 ↑\n\n下降\n湯玉琦：1 → 2 ↓\n林宜靜：3 → 6 ↓\n周美蓁：6 → 8 ↓\n許喬恩：7 → 9 ↓\n莉莉（新人）：8 → 10 ↓\n高如郁：10 → 12 ↓\n李玲玲：11 → 13 ↓\n王梅慧：12 → 14 ↓\n江麗勉：14 → 17 ↓\n陳百玲（新人）：17 → 18 ↓\n鄭珮恩：16 → 19 ↓\n謝啟芳：15 → 20 ↓\n陳玲華：20 → 21 ↓\n江沛林：19 → 22 ↓\n\n持平\n王珍珠：5 → 5 ＝\n蘇淑玲：23 → 23 ＝\n鄭上官：24 → 24 ＝\n\n六、A1／A2／B／C 分級\n\n🔴 A1｜高優先主力\n馬秋香\n湯玉琦\n林沛昕\n廖姿惠\n\n🟠 A2｜次主力追進\n王珍珠\n林宜靜\n高美雲\n周美蓁\n許喬恩\n莉莉（新人）\n徐華妤\n高如郁\n\n🟡 B組｜一般量單\n李玲玲\n王梅慧\n梁依萍\n林佩君\n江麗勉\n陳百玲（新人）\n鄭珮恩\n謝啟芳\n\n🟢 C組｜補位／觀察\n陳玲華\n江沛林\n蘇淑玲\n鄭上官\n\n七、每人一句建議\n\n1、馬秋香：你這輪靠追續金額、總業績與客單價全面拉高，今天重點是把實收補上。\n2、湯玉琦：你實收仍是全場第一，今天只要再補追續量就能穩住前段。\n3、林沛昕：你追續單數與實收都有撐住，今天是繼續往前推的關鍵。\n4、廖姿惠：你這輪客單價與追續金額明顯拉升，今天要把實收接起來。\n5、王珍珠：你單數有量、總業績有底，今天差的是把金額與實收再拉高。\n6、林宜靜：你有實收支撐，今天補上追續成交就能再往前。\n7、高美雲：你這輪名次明顯上升，今天要把成交穩定度延續。\n8、周美蓁：你有乾淨實收，今天只要再補一筆就能再動名次。\n9、許喬恩：你跟周美蓁分數相同，今天補單就能拉開差距。\n10、莉莉（新人）：你有實收亮點，今天先求穩定再往前推。\n11、徐華妤：你客單價很漂亮，今天關鍵是把實收補起來。\n12、高如郁：你有實收基礎，今天再補追續金額就能上推。\n13、李玲玲：你有單數但分數偏散，今天要提高追續金額。\n14、王梅慧：你單數有基本盤，今天差的是更高客單與實收。\n15、梁依萍：你有高客單切入點，今天先把下一筆成交接起來。\n16、林佩君：你這輪有有效分數，今天要把單數補上。\n17、江麗勉：你有追續成交，今天再補一筆就能拉高排序。\n18、陳百玲（新人）：你有累積，不急著衝，先把成交穩定做出來。\n19、鄭珮恩：你分數差距不大，今天先把追續金額補強。\n20、謝啟芳：你有成交但分數偏低，今天要先提高客單。\n21、陳玲華：你有總業績但缺追續與實收，今天先求有效成交。\n22、江沛林：先把追續單與實收補起來，排名才有上升空間。\n23、蘇淑玲：今天先求破零，有分數才有派單空間。\n24、鄭上官：先解除空白狀態，後續才有排名意義。",
    "audit": {
      "result": "PASS",
      "rule": "先審計，後排序，再派單",
      "platforms": [
        {
          "platformKey": "sanli_yixin",
          "platformName": "三立奕心",
          "passed": true,
          "metrics": {
            "累積總派單數": 0,
            "累積派單總成交數": 0,
            "累積追續總成交數": 45,
            "當日續單金額": 0,
            "本月業績": 717358,
            "追續單總金額": 575440,
            "實收總金額": 60460,
            "追續單成交": 45,
            "全部總業績": 717358,
            "追續單金額": 575440
          }
        },
        {
          "platformKey": "ftv",
          "platformName": "民視產品",
          "passed": true,
          "metrics": {
            "累積總派單數": 0,
            "累積派單總成交數": 0,
            "累積追續總成交數": 12,
            "當日續單金額": 0,
            "本月業績": 490000,
            "追續單總金額": 80590,
            "實收總金額": 18160,
            "追續單成交": 12,
            "全部總業績": 490000,
            "追續單金額": 80590
          }
        },
        {
          "platformKey": "company_product",
          "platformName": "公司產品",
          "passed": true,
          "metrics": {
            "累積總派單數": 0,
            "累積派單總成交數": 0,
            "累積追續總成交數": 4,
            "當日續單金額": 0,
            "本月業績": 45710,
            "追續單總金額": 43230,
            "實收總金額": 35250,
            "追續單成交": 4,
            "全部總業績": 45710,
            "追續單金額": 43230
          }
        }
      ],
      "notes": [
        "本輪三平台總表與個別明細加總一致。",
        "無漏算、無多算、無總盤衝突。"
      ],
      "excludedEmployees": []
    },
    "summaryBoard": {
      "累積總派單數": 0,
      "累積派單總成交數": 0,
      "累積追續總成交數": 61,
      "當日續單金額": 0,
      "本月業績": 1253068,
      "追續單總金額": 699260,
      "實收總金額": 113870,
      "當日取消退貨": 0,
      "追續單成交": 61,
      "全部總業績": 1253068,
      "追續單金額": 699260
    },
    "rankings": [
      {
        "rank": 1,
        "name": "馬秋香",
        "prevRank": 2,
        "movement": "up",
        "isNew": false,
        "group": "A1",
        "metrics": {
          "正式權重分數": 7000,
          "實收": 0,
          "續單金額": 189510,
          "總業績": 244150,
          "追續客單價": 27072.86,
          "追續成交總數": 7,
          "追續金額": 189510,
          "全部總業績": 244150,
          "追續單數": 7
        },
        "advice": "你這輪靠追續金額、總業績與客單價全面拉高，今天重點是把實收補上。"
      },
      {
        "rank": 2,
        "name": "湯玉琦",
        "prevRank": 1,
        "movement": "down",
        "isNew": false,
        "group": "A1",
        "metrics": {
          "正式權重分數": 6253.76,
          "實收": 30480,
          "續單金額": 62860,
          "總業績": 106860,
          "追續客單價": 12572,
          "追續成交總數": 5,
          "追續金額": 62860,
          "全部總業績": 106860,
          "追續單數": 5
        },
        "advice": "你實收仍是全場第一，今天只要再補追續量就能穩住前段。"
      },
      {
        "rank": 3,
        "name": "林沛昕",
        "prevRank": 4,
        "movement": "up",
        "isNew": false,
        "group": "A1",
        "metrics": {
          "正式權重分數": 6073.93,
          "實收": 18100,
          "續單金額": 95580,
          "總業績": 126148,
          "追續客單價": 13654.29,
          "追續成交總數": 7,
          "追續金額": 95580,
          "全部總業績": 126148,
          "追續單數": 7
        },
        "advice": "你追續單數與實收都有撐住，今天是繼續往前推的關鍵。"
      },
      {
        "rank": 4,
        "name": "廖姿惠",
        "prevRank": 9,
        "movement": "up",
        "isNew": false,
        "group": "A1",
        "metrics": {
          "正式權重分數": 3661.65,
          "實收": 2980,
          "續單金額": 70660,
          "總業績": 97710,
          "追續客單價": 17665,
          "追續成交總數": 4,
          "追續金額": 70660,
          "全部總業績": 97710,
          "追續單數": 4
        },
        "advice": "你這輪客單價與追續金額明顯拉升，今天要把實收接起來。"
      },
      {
        "rank": 5,
        "name": "王珍珠",
        "prevRank": 5,
        "movement": "flat",
        "isNew": false,
        "group": "A2",
        "metrics": {
          "正式權重分數": 3188.03,
          "實收": 0,
          "續單金額": 42650,
          "總業績": 128230,
          "追續客單價": 6092.86,
          "追續成交總數": 7,
          "追續金額": 42650,
          "全部總業績": 128230,
          "追續單數": 7
        },
        "advice": "你單數有量、總業績有底，今天差的是把金額與實收再拉高。"
      },
      {
        "rank": 6,
        "name": "林宜靜",
        "prevRank": 3,
        "movement": "down",
        "isNew": false,
        "group": "A2",
        "metrics": {
          "正式權重分數": 3077.17,
          "實收": 7600,
          "續單金額": 31380,
          "總業績": 112760,
          "追續客單價": 10460,
          "追續成交總數": 3,
          "追續金額": 31380,
          "全部總業績": 112760,
          "追續單數": 3
        },
        "advice": "你有實收支撐，今天補上追續成交就能再往前。"
      },
      {
        "rank": 7,
        "name": "高美雲",
        "prevRank": 13,
        "movement": "up",
        "isNew": false,
        "group": "A2",
        "metrics": {
          "正式權重分數": 2930.5,
          "實收": 11250,
          "續單金額": 26070,
          "總業績": 42490,
          "追續客單價": 6517.5,
          "追續成交總數": 4,
          "追續金額": 26070,
          "全部總業績": 42490,
          "追續單數": 4
        },
        "advice": "你這輪名次明顯上升，今天要把成交穩定度延續。"
      },
      {
        "rank": 8,
        "name": "周美蓁",
        "prevRank": 6,
        "movement": "down",
        "isNew": false,
        "group": "A2",
        "metrics": {
          "正式權重分數": 2292.29,
          "實收": 12000,
          "續單金額": 12000,
          "總業績": 12000,
          "追續客單價": 12000,
          "追續成交總數": 1,
          "追續金額": 12000,
          "全部總業績": 12000,
          "追續單數": 1
        },
        "advice": "你有乾淨實收，今天只要再補一筆就能再動名次。"
      },
      {
        "rank": 9,
        "name": "許喬恩",
        "prevRank": 7,
        "movement": "down",
        "isNew": false,
        "group": "A2",
        "metrics": {
          "正式權重分數": 2292.29,
          "實收": 12000,
          "續單金額": 12000,
          "總業績": 12000,
          "追續客單價": 12000,
          "追續成交總數": 1,
          "追續金額": 12000,
          "全部總業績": 12000,
          "追續單數": 1
        },
        "advice": "你跟周美蓁分數相同，今天補單就能拉開差距。"
      },
      {
        "rank": 10,
        "name": "莉莉（新人）",
        "prevRank": 8,
        "movement": "down",
        "isNew": true,
        "group": "A2",
        "metrics": {
          "正式權重分數": 2271.51,
          "實收": 11880,
          "續單金額": 11880,
          "總業績": 11880,
          "追續客單價": 11880,
          "追續成交總數": 1,
          "追續金額": 11880,
          "全部總業績": 11880,
          "追續單數": 1
        },
        "advice": "你有實收亮點，今天先求穩定再往前推。"
      },
      {
        "rank": 11,
        "name": "徐華妤",
        "prevRank": 21,
        "movement": "up",
        "isNew": false,
        "group": "A2",
        "metrics": {
          "正式權重分數": 2105.03,
          "實收": 0,
          "續單金額": 35640,
          "總業績": 35640,
          "追續客單價": 17820,
          "追續成交總數": 2,
          "追續金額": 35640,
          "全部總業績": 35640,
          "追續單數": 2
        },
        "advice": "你客單價很漂亮，今天關鍵是把實收補起來。"
      },
      {
        "rank": 12,
        "name": "高如郁",
        "prevRank": 10,
        "movement": "down",
        "isNew": false,
        "group": "A2",
        "metrics": {
          "正式權重分數": 2039.76,
          "實收": 7580,
          "續單金額": 15500,
          "總業績": 37640,
          "追續客單價": 7750,
          "追續成交總數": 2,
          "追續金額": 15500,
          "全部總業績": 37640,
          "追續單數": 2
        },
        "advice": "你有實收基礎，今天再補追續金額就能上推。"
      },
      {
        "rank": 13,
        "name": "李玲玲",
        "prevRank": 11,
        "movement": "down",
        "isNew": false,
        "group": "B",
        "metrics": {
          "正式權重分數": 1842.52,
          "實收": 0,
          "續單金額": 18560,
          "總業績": 78690,
          "追續客單價": 4640,
          "追續成交總數": 4,
          "追續金額": 18560,
          "全部總業績": 78690,
          "追續單數": 4
        },
        "advice": "你有單數但分數偏散，今天要提高追續金額。"
      },
      {
        "rank": 14,
        "name": "王梅慧",
        "prevRank": 12,
        "movement": "down",
        "isNew": false,
        "group": "B",
        "metrics": {
          "正式權重分數": 1742.81,
          "實收": 0,
          "續單金額": 20800,
          "總業績": 52600,
          "追續客單價": 5200,
          "追續成交總數": 4,
          "追續金額": 20800,
          "全部總業績": 52600,
          "追續單數": 4
        },
        "advice": "你單數有基本盤，今天差的是更高客單與實收。"
      },
      {
        "rank": 15,
        "name": "梁依萍",
        "prevRank": 18,
        "movement": "up",
        "isNew": false,
        "group": "B",
        "metrics": {
          "正式權重分數": 1486.55,
          "實收": 0,
          "續單金額": 14280,
          "總業績": 47640,
          "追續客單價": 14280,
          "追續成交總數": 1,
          "追續金額": 14280,
          "全部總業績": 47640,
          "追續單數": 1
        },
        "advice": "你有高客單切入點，今天先把下一筆成交接起來。"
      },
      {
        "rank": 16,
        "name": "林佩君",
        "prevRank": 22,
        "movement": "up",
        "isNew": false,
        "group": "B",
        "metrics": {
          "正式權重分數": 1312.99,
          "實收": 0,
          "續單金額": 14700,
          "總業績": 14700,
          "追續客單價": 14700,
          "追續成交總數": 1,
          "追續金額": 14700,
          "全部總業績": 14700,
          "追續單數": 1
        },
        "advice": "你這輪有有效分數，今天要把單數補上。"
      },
      {
        "rank": 17,
        "name": "江麗勉",
        "prevRank": 14,
        "movement": "down",
        "isNew": false,
        "group": "B",
        "metrics": {
          "正式權重分數": 898.95,
          "實收": 0,
          "續單金額": 9480,
          "總業績": 13460,
          "追續客單價": 4740,
          "追續成交總數": 2,
          "追續金額": 9480,
          "全部總業績": 13460,
          "追續單數": 2
        },
        "advice": "你有追續成交，今天再補一筆就能拉高排序。"
      },
      {
        "rank": 18,
        "name": "陳百玲（新人）",
        "prevRank": 17,
        "movement": "down",
        "isNew": true,
        "group": "B",
        "metrics": {
          "正式權重分數": 756.9,
          "實收": 0,
          "續單金額": 6980,
          "總業績": 6980,
          "追續客單價": 3490,
          "追續成交總數": 2,
          "追續金額": 6980,
          "全部總業績": 6980,
          "追續單數": 2
        },
        "advice": "你有累積，不急著衝，先把成交穩定做出來。"
      },
      {
        "rank": 19,
        "name": "鄭珮恩",
        "prevRank": 16,
        "movement": "down",
        "isNew": false,
        "group": "B",
        "metrics": {
          "正式權重分數": 752.8,
          "實收": 0,
          "續單金額": 5750,
          "總業績": 14500,
          "追續客單價": 2875,
          "追續成交總數": 2,
          "追續金額": 5750,
          "全部總業績": 14500,
          "追續單數": 2
        },
        "advice": "你分數差距不大，今天先把追續金額補強。"
      },
      {
        "rank": 20,
        "name": "謝啟芳",
        "prevRank": 15,
        "movement": "down",
        "isNew": false,
        "group": "B",
        "metrics": {
          "正式權重分數": 518.85,
          "實收": 0,
          "續單金額": 2980,
          "總業績": 16300,
          "追續客單價": 2980,
          "追續成交總數": 1,
          "追續金額": 2980,
          "全部總業績": 16300,
          "追續單數": 1
        },
        "advice": "你有成交但分數偏低，今天要先提高客單。"
      },
      {
        "rank": 21,
        "name": "陳玲華",
        "prevRank": 20,
        "movement": "down",
        "isNew": false,
        "group": "C",
        "metrics": {
          "正式權重分數": 158.14,
          "實收": 0,
          "續單金額": 0,
          "總業績": 25740,
          "追續客單價": 0,
          "追續成交總數": 0,
          "追續金額": 0,
          "全部總業績": 25740,
          "追續單數": 0
        },
        "advice": "你有總業績但缺追續與實收，今天先求有效成交。"
      },
      {
        "rank": 22,
        "name": "江沛林",
        "prevRank": 19,
        "movement": "down",
        "isNew": false,
        "group": "C",
        "metrics": {
          "正式權重分數": 91.85,
          "實收": 0,
          "續單金額": 0,
          "總業績": 14950,
          "追續客單價": 0,
          "追續成交總數": 0,
          "追續金額": 0,
          "全部總業績": 14950,
          "追續單數": 0
        },
        "advice": "先把追續單與實收補起來，排名才有上升空間。"
      },
      {
        "rank": 23,
        "name": "蘇淑玲",
        "prevRank": 23,
        "movement": "flat",
        "isNew": false,
        "group": "C",
        "metrics": {
          "正式權重分數": 0,
          "實收": 0,
          "續單金額": 0,
          "總業績": 0,
          "追續客單價": 0,
          "追續成交總數": 0,
          "追續金額": 0,
          "全部總業績": 0,
          "追續單數": 0
        },
        "advice": "今天先求破零，有分數才有派單空間。"
      },
      {
        "rank": 24,
        "name": "鄭上官",
        "prevRank": 24,
        "movement": "flat",
        "isNew": false,
        "group": "C",
        "metrics": {
          "正式權重分數": 0,
          "實收": 0,
          "續單金額": 0,
          "總業績": 0,
          "追續客單價": 0,
          "追續成交總數": 0,
          "追續金額": 0,
          "全部總業績": 0,
          "追續單數": 0
        },
        "advice": "先解除空白狀態，後續才有排名意義。"
      }
    ],
    "groups": {
      "A1": [
        "馬秋香",
        "湯玉琦",
        "林沛昕",
        "廖姿惠"
      ],
      "A2": [
        "王珍珠",
        "林宜靜",
        "高美雲",
        "周美蓁",
        "許喬恩",
        "莉莉（新人）",
        "徐華妤",
        "高如郁"
      ],
      "B": [
        "李玲玲",
        "王梅慧",
        "梁依萍",
        "林佩君",
        "江麗勉",
        "陳百玲（新人）",
        "鄭珮恩",
        "謝啟芳"
      ],
      "C": [
        "陳玲華",
        "江沛林",
        "蘇淑玲",
        "鄭上官"
      ]
    },
    "adviceList": [
      {
        "name": "馬秋香",
        "rank": 1,
        "group": "A1",
        "text": "你這輪靠追續金額、總業績與客單價全面拉高，今天重點是把實收補上。"
      },
      {
        "name": "湯玉琦",
        "rank": 2,
        "group": "A1",
        "text": "你實收仍是全場第一，今天只要再補追續量就能穩住前段。"
      },
      {
        "name": "林沛昕",
        "rank": 3,
        "group": "A1",
        "text": "你追續單數與實收都有撐住，今天是繼續往前推的關鍵。"
      },
      {
        "name": "廖姿惠",
        "rank": 4,
        "group": "A1",
        "text": "你這輪客單價與追續金額明顯拉升，今天要把實收接起來。"
      },
      {
        "name": "王珍珠",
        "rank": 5,
        "group": "A2",
        "text": "你單數有量、總業績有底，今天差的是把金額與實收再拉高。"
      },
      {
        "name": "林宜靜",
        "rank": 6,
        "group": "A2",
        "text": "你有實收支撐，今天補上追續成交就能再往前。"
      },
      {
        "name": "高美雲",
        "rank": 7,
        "group": "A2",
        "text": "你這輪名次明顯上升，今天要把成交穩定度延續。"
      },
      {
        "name": "周美蓁",
        "rank": 8,
        "group": "A2",
        "text": "你有乾淨實收，今天只要再補一筆就能再動名次。"
      },
      {
        "name": "許喬恩",
        "rank": 9,
        "group": "A2",
        "text": "你跟周美蓁分數相同，今天補單就能拉開差距。"
      },
      {
        "name": "莉莉（新人）",
        "rank": 10,
        "group": "A2",
        "text": "你有實收亮點，今天先求穩定再往前推。"
      },
      {
        "name": "徐華妤",
        "rank": 11,
        "group": "A2",
        "text": "你客單價很漂亮，今天關鍵是把實收補起來。"
      },
      {
        "name": "高如郁",
        "rank": 12,
        "group": "A2",
        "text": "你有實收基礎，今天再補追續金額就能上推。"
      },
      {
        "name": "李玲玲",
        "rank": 13,
        "group": "B",
        "text": "你有單數但分數偏散，今天要提高追續金額。"
      },
      {
        "name": "王梅慧",
        "rank": 14,
        "group": "B",
        "text": "你單數有基本盤，今天差的是更高客單與實收。"
      },
      {
        "name": "梁依萍",
        "rank": 15,
        "group": "B",
        "text": "你有高客單切入點，今天先把下一筆成交接起來。"
      },
      {
        "name": "林佩君",
        "rank": 16,
        "group": "B",
        "text": "你這輪有有效分數，今天要把單數補上。"
      },
      {
        "name": "江麗勉",
        "rank": 17,
        "group": "B",
        "text": "你有追續成交，今天再補一筆就能拉高排序。"
      },
      {
        "name": "陳百玲（新人）",
        "rank": 18,
        "group": "B",
        "text": "你有累積，不急著衝，先把成交穩定做出來。"
      },
      {
        "name": "鄭珮恩",
        "rank": 19,
        "group": "B",
        "text": "你分數差距不大，今天先把追續金額補強。"
      },
      {
        "name": "謝啟芳",
        "rank": 20,
        "group": "B",
        "text": "你有成交但分數偏低，今天要先提高客單。"
      },
      {
        "name": "陳玲華",
        "rank": 21,
        "group": "C",
        "text": "你有總業績但缺追續與實收，今天先求有效成交。"
      },
      {
        "name": "江沛林",
        "rank": 22,
        "group": "C",
        "text": "先把追續單與實收補起來，排名才有上升空間。"
      },
      {
        "name": "蘇淑玲",
        "rank": 23,
        "group": "C",
        "text": "今天先求破零，有分數才有派單空間。"
      },
      {
        "name": "鄭上官",
        "rank": 24,
        "group": "C",
        "text": "先解除空白狀態，後續才有排名意義。"
      }
    ],
    "finalConfirmations": [
      "5/4 最新業績已統一為正式版。",
      "三平台總表核對通過。",
      "正式名次、分級與公告文字以本資料為準。"
    ],
    "groupShortText": "📣【AI 派單公告｜5/4 結算 → 5/5 正式派單順序｜AI 比例原則版】\n\n一、審計結論\n\n審計結果：PASS\n本輪已依鎖死規則執行：先審計，後運算，後排序，再派單。\n\n三立奕心總表核對通過\n追續單成交：45 ✅\n全部總業績：717,358 ✅\n追續單金額：575,440 ✅\n實收總金額：60,460 ✅\n\n民視產品總表核對通過\n追續單成交：12 ✅\n全部總業績：490,000 ✅\n追續單金額：80,590 ✅\n實收總金額：18,160 ✅\n\n公司產品總表核對通過\n追續單成交：4 ✅\n全部總業績：45,710 ✅\n追續單金額：43,230 ✅\n實收總金額：35,250 ✅\n\n異常與提醒\n本輪三平台總表與個別明細加總一致。\n無漏算、無多算、無總盤衝突。\n\n二、整合總盤\n\n【追續單成交】61\n【全部總業績】1,253,068\n【追續單金額】699,260\n【實收總金額】113,870\n\n三、AI 比例原則\n\n本輪權重：\n實收總業績：3000\n追續單金額：2500\n全部總金額：1500\n追續客單價：1500\n追續單數量：1500\n\n正式權重分數＝\n實收比例分數＋追續金額比例分數＋全部總業績比例分數＋追續客單價比例分數＋追續單數量比例分數\n\n本輪全員最高值：\n實收總金額最高：30,480（湯玉琦）\n追續單金額最高：189,510（馬秋香）\n全部總業績最高：244,150（馬秋香）\n追續客單價最高：27,072.86（馬秋香）\n追續單數量最高：7（王珍珠、馬秋香、林沛昕）\n\n四、正式名次\n\n1、馬秋香｜【正式權重分數】7000.00｜【實收】0｜【追續金額】189,510｜【全部總業績】244,150｜【追續客單價】27,072.86｜【追續單數】7\n2、湯玉琦｜【正式權重分數】6253.76｜【實收】30,480｜【追續金額】62,860｜【全部總業績】106,860｜【追續客單價】12,572.00｜【追續單數】5\n3、林沛昕｜【正式權重分數】6073.93｜【實收】18,100｜【追續金額】95,580｜【全部總業績】126,148｜【追續客單價】13,654.29｜【追續單數】7\n4、廖姿惠｜【正式權重分數】3661.65｜【實收】2,980｜【追續金額】70,660｜【全部總業績】97,710｜【追續客單價】17,665.00｜【追續單數】4\n5、王珍珠｜【正式權重分數】3188.03｜【實收】0｜【追續金額】42,650｜【全部總業績】128,230｜【追續客單價】6,092.86｜【追續單數】7\n6、林宜靜｜【正式權重分數】3077.17｜【實收】7,600｜【追續金額】31,380｜【全部總業績】112,760｜【追續客單價】10,460.00｜【追續單數】3\n7、高美雲｜【正式權重分數】2930.50｜【實收】11,250｜【追續金額】26,070｜【全部總業績】42,490｜【追續客單價】6,517.50｜【追續單數】4\n8、周美蓁｜【正式權重分數】2292.29｜【實收】12,000｜【追續金額】12,000｜【全部總業績】12,000｜【追續客單價】12,000.00｜【追續單數】1\n9、許喬恩｜【正式權重分數】2292.29｜【實收】12,000｜【追續金額】12,000｜【全部總業績】12,000｜【追續客單價】12,000.00｜【追續單數】1\n10、莉莉（新人）｜【正式權重分數】2271.51｜【實收】11,880｜【追續金額】11,880｜【全部總業績】11,880｜【追續客單價】11,880.00｜【追續單數】1\n11、徐華妤｜【正式權重分數】2105.03｜【實收】0｜【追續金額】35,640｜【全部總業績】35,640｜【追續客單價】17,820.00｜【追續單數】2\n12、高如郁｜【正式權重分數】2039.76｜【實收】7,580｜【追續金額】15,500｜【全部總業績】37,640｜【追續客單價】7,750.00｜【追續單數】2\n13、李玲玲｜【正式權重分數】1842.52｜【實收】0｜【追續金額】18,560｜【全部總業績】78,690｜【追續客單價】4,640.00｜【追續單數】4\n14、王梅慧｜【正式權重分數】1742.81｜【實收】0｜【追續金額】20,800｜【全部總業績】52,600｜【追續客單價】5,200.00｜【追續單數】4\n15、梁依萍｜【正式權重分數】1486.55｜【實收】0｜【追續金額】14,280｜【全部總業績】47,640｜【追續客單價】14,280.00｜【追續單數】1\n16、林佩君｜【正式權重分數】1312.99｜【實收】0｜【追續金額】14,700｜【全部總業績】14,700｜【追續客單價】14,700.00｜【追續單數】1\n17、江麗勉｜【正式權重分數】898.95｜【實收】0｜【追續金額】9,480｜【全部總業績】13,460｜【追續客單價】4,740.00｜【追續單數】2\n18、陳百玲（新人）｜【正式權重分數】756.90｜【實收】0｜【追續金額】6,980｜【全部總業績】6,980｜【追續客單價】3,490.00｜【追續單數】2\n19、鄭珮恩｜【正式權重分數】752.80｜【實收】0｜【追續金額】5,750｜【全部總業績】14,500｜【追續客單價】2,875.00｜【追續單數】2\n20、謝啟芳｜【正式權重分數】518.85｜【實收】0｜【追續金額】2,980｜【全部總業績】16,300｜【追續客單價】2,980.00｜【追續單數】1\n21、陳玲華｜【正式權重分數】158.14｜【實收】0｜【追續金額】0｜【全部總業績】25,740｜【追續客單價】0｜【追續單數】0\n22、江沛林｜【正式權重分數】91.85｜【實收】0｜【追續金額】0｜【全部總業績】14,950｜【追續客單價】0｜【追續單數】0\n23、蘇淑玲｜【正式權重分數】0｜【實收】0｜【追續金額】0｜【全部總業績】0｜【追續客單價】0｜【追續單數】0\n24、鄭上官｜【正式權重分數】0｜【實收】0｜【追續金額】0｜【全部總業績】0｜【追續客單價】0｜【追續單數】0\n\n五、名次異動\n\n以上一輪 5/3 正式派單名次對照，本輪異動如下：\n\n上升\n馬秋香：2 → 1 ↑\n林沛昕：4 → 3 ↑\n廖姿惠：9 → 4 ↑\n高美雲：13 → 7 ↑\n徐華妤：21 → 11 ↑\n梁依萍：18 → 15 ↑\n林佩君：22 → 16 ↑\n\n下降\n湯玉琦：1 → 2 ↓\n林宜靜：3 → 6 ↓\n周美蓁：6 → 8 ↓\n許喬恩：7 → 9 ↓\n莉莉（新人）：8 → 10 ↓\n高如郁：10 → 12 ↓\n李玲玲：11 → 13 ↓\n王梅慧：12 → 14 ↓\n江麗勉：14 → 17 ↓\n陳百玲（新人）：17 → 18 ↓\n鄭珮恩：16 → 19 ↓\n謝啟芳：15 → 20 ↓\n陳玲華：20 → 21 ↓\n江沛林：19 → 22 ↓\n\n持平\n王珍珠：5 → 5 ＝\n蘇淑玲：23 → 23 ＝\n鄭上官：24 → 24 ＝\n\n六、A1／A2／B／C 分級\n\n🔴 A1｜高優先主力\n馬秋香\n湯玉琦\n林沛昕\n廖姿惠\n\n🟠 A2｜次主力追進\n王珍珠\n林宜靜\n高美雲\n周美蓁\n許喬恩\n莉莉（新人）\n徐華妤\n高如郁\n\n🟡 B組｜一般量單\n李玲玲\n王梅慧\n梁依萍\n林佩君\n江麗勉\n陳百玲（新人）\n鄭珮恩\n謝啟芳\n\n🟢 C組｜補位／觀察\n陳玲華\n江沛林\n蘇淑玲\n鄭上官\n\n七、每人一句建議\n\n1、馬秋香：你這輪靠追續金額、總業績與客單價全面拉高，今天重點是把實收補上。\n2、湯玉琦：你實收仍是全場第一，今天只要再補追續量就能穩住前段。\n3、林沛昕：你追續單數與實收都有撐住，今天是繼續往前推的關鍵。\n4、廖姿惠：你這輪客單價與追續金額明顯拉升，今天要把實收接起來。\n5、王珍珠：你單數有量、總業績有底，今天差的是把金額與實收再拉高。\n6、林宜靜：你有實收支撐，今天補上追續成交就能再往前。\n7、高美雲：你這輪名次明顯上升，今天要把成交穩定度延續。\n8、周美蓁：你有乾淨實收，今天只要再補一筆就能再動名次。\n9、許喬恩：你跟周美蓁分數相同，今天補單就能拉開差距。\n10、莉莉（新人）：你有實收亮點，今天先求穩定再往前推。\n11、徐華妤：你客單價很漂亮，今天關鍵是把實收補起來。\n12、高如郁：你有實收基礎，今天再補追續金額就能上推。\n13、李玲玲：你有單數但分數偏散，今天要提高追續金額。\n14、王梅慧：你單數有基本盤，今天差的是更高客單與實收。\n15、梁依萍：你有高客單切入點，今天先把下一筆成交接起來。\n16、林佩君：你這輪有有效分數，今天要把單數補上。\n17、江麗勉：你有追續成交，今天再補一筆就能拉高排序。\n18、陳百玲（新人）：你有累積，不急著衝，先把成交穩定做出來。\n19、鄭珮恩：你分數差距不大，今天先把追續金額補強。\n20、謝啟芳：你有成交但分數偏低，今天要先提高客單。\n21、陳玲華：你有總業績但缺追續與實收，今天先求有效成交。\n22、江沛林：先把追續單與實收補起來，排名才有上升空間。\n23、蘇淑玲：今天先求破零，有分數才有派單空間。\n24、鄭上官：先解除空白狀態，後續才有排名意義。\n\n正式前10名：1馬秋香 2湯玉琦 3林沛昕 4廖姿惠 5王珍珠 6林宜靜 7高美雲 8周美蓁 9許喬恩 10莉莉（新人）。\nA1：馬秋香、湯玉琦、林沛昕、廖姿惠。\nA2：王珍珠、林宜靜、高美雲、周美蓁、許喬恩、莉莉（新人）、徐華妤、高如郁。\nB組：李玲玲、王梅慧、梁依萍、林佩君、江麗勉、陳百玲（新人）、鄭珮恩、謝啟芳。\nC組：陳玲華、江沛林、蘇淑玲、鄭上官。"
  },
  "meta": {
    "operator": "system-super-optimizer",
    "savedAt": "2026-05-04T11:27:45.671Z",
    "reason": "Official 5/4 settlement to 5/5 dispatch unified update",
    "source": "manual-official"
  },
  "standardData": {
    "公告標題": "AI 派單公告｜5/4 結算 → 5/5 正式派單",
    "交接資料": {
      "結算日": "5/4",
      "派單日": "5/5"
    },
    "審計結論": {
      "result": "PASS",
      "rule": "先審計，後排序，再派單",
      "platforms": [
        {
          "platformKey": "sanli_yixin",
          "platformName": "三立奕心",
          "passed": true,
          "metrics": {
            "累積總派單數": 0,
            "累積派單總成交數": 0,
            "累積追續總成交數": 45,
            "當日續單金額": 0,
            "本月業績": 717358,
            "追續單總金額": 575440,
            "實收總金額": 60460,
            "追續單成交": 45,
            "全部總業績": 717358,
            "追續單金額": 575440
          }
        },
        {
          "platformKey": "ftv",
          "platformName": "民視產品",
          "passed": true,
          "metrics": {
            "累積總派單數": 0,
            "累積派單總成交數": 0,
            "累積追續總成交數": 12,
            "當日續單金額": 0,
            "本月業績": 490000,
            "追續單總金額": 80590,
            "實收總金額": 18160,
            "追續單成交": 12,
            "全部總業績": 490000,
            "追續單金額": 80590
          }
        },
        {
          "platformKey": "company_product",
          "platformName": "公司產品",
          "passed": true,
          "metrics": {
            "累積總派單數": 0,
            "累積派單總成交數": 0,
            "累積追續總成交數": 4,
            "當日續單金額": 0,
            "本月業績": 45710,
            "追續單總金額": 43230,
            "實收總金額": 35250,
            "追續單成交": 4,
            "全部總業績": 45710,
            "追續單金額": 43230
          }
        }
      ],
      "notes": [
        "本輪三平台總表與個別明細加總一致。",
        "無漏算、無多算、無總盤衝突。"
      ],
      "excludedEmployees": []
    },
    "整合總盤": {
      "累積總派單數": 0,
      "累積派單總成交數": 0,
      "累積追續總成交數": 61,
      "當日續單金額": 0,
      "本月業績": 1253068,
      "追續單總金額": 699260,
      "實收總金額": 113870,
      "當日取消退貨": 0,
      "追續單成交": 61,
      "全部總業績": 1253068,
      "追續單金額": 699260
    },
    "正式名次": [
      {
        "名次": 1,
        "姓名": "馬秋香",
        "分級": "A1",
        "上輪名次": 2,
        "異動": "up",
        "正式權重分數": 7000,
        "實收": 0,
        "續單金額": 189510,
        "總業績": 244150,
        "追續客單價": 27072.86,
        "追續成交總數": 7,
        "追續金額": 189510,
        "全部總業績": 244150,
        "追續單數": 7,
        "建議": "你這輪靠追續金額、總業績與客單價全面拉高，今天重點是把實收補上。"
      },
      {
        "名次": 2,
        "姓名": "湯玉琦",
        "分級": "A1",
        "上輪名次": 1,
        "異動": "down",
        "正式權重分數": 6253.76,
        "實收": 30480,
        "續單金額": 62860,
        "總業績": 106860,
        "追續客單價": 12572,
        "追續成交總數": 5,
        "追續金額": 62860,
        "全部總業績": 106860,
        "追續單數": 5,
        "建議": "你實收仍是全場第一，今天只要再補追續量就能穩住前段。"
      },
      {
        "名次": 3,
        "姓名": "林沛昕",
        "分級": "A1",
        "上輪名次": 4,
        "異動": "up",
        "正式權重分數": 6073.93,
        "實收": 18100,
        "續單金額": 95580,
        "總業績": 126148,
        "追續客單價": 13654.29,
        "追續成交總數": 7,
        "追續金額": 95580,
        "全部總業績": 126148,
        "追續單數": 7,
        "建議": "你追續單數與實收都有撐住，今天是繼續往前推的關鍵。"
      },
      {
        "名次": 4,
        "姓名": "廖姿惠",
        "分級": "A1",
        "上輪名次": 9,
        "異動": "up",
        "正式權重分數": 3661.65,
        "實收": 2980,
        "續單金額": 70660,
        "總業績": 97710,
        "追續客單價": 17665,
        "追續成交總數": 4,
        "追續金額": 70660,
        "全部總業績": 97710,
        "追續單數": 4,
        "建議": "你這輪客單價與追續金額明顯拉升，今天要把實收接起來。"
      },
      {
        "名次": 5,
        "姓名": "王珍珠",
        "分級": "A2",
        "上輪名次": 5,
        "異動": "flat",
        "正式權重分數": 3188.03,
        "實收": 0,
        "續單金額": 42650,
        "總業績": 128230,
        "追續客單價": 6092.86,
        "追續成交總數": 7,
        "追續金額": 42650,
        "全部總業績": 128230,
        "追續單數": 7,
        "建議": "你單數有量、總業績有底，今天差的是把金額與實收再拉高。"
      },
      {
        "名次": 6,
        "姓名": "林宜靜",
        "分級": "A2",
        "上輪名次": 3,
        "異動": "down",
        "正式權重分數": 3077.17,
        "實收": 7600,
        "續單金額": 31380,
        "總業績": 112760,
        "追續客單價": 10460,
        "追續成交總數": 3,
        "追續金額": 31380,
        "全部總業績": 112760,
        "追續單數": 3,
        "建議": "你有實收支撐，今天補上追續成交就能再往前。"
      },
      {
        "名次": 7,
        "姓名": "高美雲",
        "分級": "A2",
        "上輪名次": 13,
        "異動": "up",
        "正式權重分數": 2930.5,
        "實收": 11250,
        "續單金額": 26070,
        "總業績": 42490,
        "追續客單價": 6517.5,
        "追續成交總數": 4,
        "追續金額": 26070,
        "全部總業績": 42490,
        "追續單數": 4,
        "建議": "你這輪名次明顯上升，今天要把成交穩定度延續。"
      },
      {
        "名次": 8,
        "姓名": "周美蓁",
        "分級": "A2",
        "上輪名次": 6,
        "異動": "down",
        "正式權重分數": 2292.29,
        "實收": 12000,
        "續單金額": 12000,
        "總業績": 12000,
        "追續客單價": 12000,
        "追續成交總數": 1,
        "追續金額": 12000,
        "全部總業績": 12000,
        "追續單數": 1,
        "建議": "你有乾淨實收，今天只要再補一筆就能再動名次。"
      },
      {
        "名次": 9,
        "姓名": "許喬恩",
        "分級": "A2",
        "上輪名次": 7,
        "異動": "down",
        "正式權重分數": 2292.29,
        "實收": 12000,
        "續單金額": 12000,
        "總業績": 12000,
        "追續客單價": 12000,
        "追續成交總數": 1,
        "追續金額": 12000,
        "全部總業績": 12000,
        "追續單數": 1,
        "建議": "你跟周美蓁分數相同，今天補單就能拉開差距。"
      },
      {
        "名次": 10,
        "姓名": "莉莉（新人）",
        "分級": "A2",
        "上輪名次": 8,
        "異動": "down",
        "正式權重分數": 2271.51,
        "實收": 11880,
        "續單金額": 11880,
        "總業績": 11880,
        "追續客單價": 11880,
        "追續成交總數": 1,
        "追續金額": 11880,
        "全部總業績": 11880,
        "追續單數": 1,
        "建議": "你有實收亮點，今天先求穩定再往前推。"
      },
      {
        "名次": 11,
        "姓名": "徐華妤",
        "分級": "A2",
        "上輪名次": 21,
        "異動": "up",
        "正式權重分數": 2105.03,
        "實收": 0,
        "續單金額": 35640,
        "總業績": 35640,
        "追續客單價": 17820,
        "追續成交總數": 2,
        "追續金額": 35640,
        "全部總業績": 35640,
        "追續單數": 2,
        "建議": "你客單價很漂亮，今天關鍵是把實收補起來。"
      },
      {
        "名次": 12,
        "姓名": "高如郁",
        "分級": "A2",
        "上輪名次": 10,
        "異動": "down",
        "正式權重分數": 2039.76,
        "實收": 7580,
        "續單金額": 15500,
        "總業績": 37640,
        "追續客單價": 7750,
        "追續成交總數": 2,
        "追續金額": 15500,
        "全部總業績": 37640,
        "追續單數": 2,
        "建議": "你有實收基礎，今天再補追續金額就能上推。"
      },
      {
        "名次": 13,
        "姓名": "李玲玲",
        "分級": "B",
        "上輪名次": 11,
        "異動": "down",
        "正式權重分數": 1842.52,
        "實收": 0,
        "續單金額": 18560,
        "總業績": 78690,
        "追續客單價": 4640,
        "追續成交總數": 4,
        "追續金額": 18560,
        "全部總業績": 78690,
        "追續單數": 4,
        "建議": "你有單數但分數偏散，今天要提高追續金額。"
      },
      {
        "名次": 14,
        "姓名": "王梅慧",
        "分級": "B",
        "上輪名次": 12,
        "異動": "down",
        "正式權重分數": 1742.81,
        "實收": 0,
        "續單金額": 20800,
        "總業績": 52600,
        "追續客單價": 5200,
        "追續成交總數": 4,
        "追續金額": 20800,
        "全部總業績": 52600,
        "追續單數": 4,
        "建議": "你單數有基本盤，今天差的是更高客單與實收。"
      },
      {
        "名次": 15,
        "姓名": "梁依萍",
        "分級": "B",
        "上輪名次": 18,
        "異動": "up",
        "正式權重分數": 1486.55,
        "實收": 0,
        "續單金額": 14280,
        "總業績": 47640,
        "追續客單價": 14280,
        "追續成交總數": 1,
        "追續金額": 14280,
        "全部總業績": 47640,
        "追續單數": 1,
        "建議": "你有高客單切入點，今天先把下一筆成交接起來。"
      },
      {
        "名次": 16,
        "姓名": "林佩君",
        "分級": "B",
        "上輪名次": 22,
        "異動": "up",
        "正式權重分數": 1312.99,
        "實收": 0,
        "續單金額": 14700,
        "總業績": 14700,
        "追續客單價": 14700,
        "追續成交總數": 1,
        "追續金額": 14700,
        "全部總業績": 14700,
        "追續單數": 1,
        "建議": "你這輪有有效分數，今天要把單數補上。"
      },
      {
        "名次": 17,
        "姓名": "江麗勉",
        "分級": "B",
        "上輪名次": 14,
        "異動": "down",
        "正式權重分數": 898.95,
        "實收": 0,
        "續單金額": 9480,
        "總業績": 13460,
        "追續客單價": 4740,
        "追續成交總數": 2,
        "追續金額": 9480,
        "全部總業績": 13460,
        "追續單數": 2,
        "建議": "你有追續成交，今天再補一筆就能拉高排序。"
      },
      {
        "名次": 18,
        "姓名": "陳百玲（新人）",
        "分級": "B",
        "上輪名次": 17,
        "異動": "down",
        "正式權重分數": 756.9,
        "實收": 0,
        "續單金額": 6980,
        "總業績": 6980,
        "追續客單價": 3490,
        "追續成交總數": 2,
        "追續金額": 6980,
        "全部總業績": 6980,
        "追續單數": 2,
        "建議": "你有累積，不急著衝，先把成交穩定做出來。"
      },
      {
        "名次": 19,
        "姓名": "鄭珮恩",
        "分級": "B",
        "上輪名次": 16,
        "異動": "down",
        "正式權重分數": 752.8,
        "實收": 0,
        "續單金額": 5750,
        "總業績": 14500,
        "追續客單價": 2875,
        "追續成交總數": 2,
        "追續金額": 5750,
        "全部總業績": 14500,
        "追續單數": 2,
        "建議": "你分數差距不大，今天先把追續金額補強。"
      },
      {
        "名次": 20,
        "姓名": "謝啟芳",
        "分級": "B",
        "上輪名次": 15,
        "異動": "down",
        "正式權重分數": 518.85,
        "實收": 0,
        "續單金額": 2980,
        "總業績": 16300,
        "追續客單價": 2980,
        "追續成交總數": 1,
        "追續金額": 2980,
        "全部總業績": 16300,
        "追續單數": 1,
        "建議": "你有成交但分數偏低，今天要先提高客單。"
      },
      {
        "名次": 21,
        "姓名": "陳玲華",
        "分級": "C",
        "上輪名次": 20,
        "異動": "down",
        "正式權重分數": 158.14,
        "實收": 0,
        "續單金額": 0,
        "總業績": 25740,
        "追續客單價": 0,
        "追續成交總數": 0,
        "追續金額": 0,
        "全部總業績": 25740,
        "追續單數": 0,
        "建議": "你有總業績但缺追續與實收，今天先求有效成交。"
      },
      {
        "名次": 22,
        "姓名": "江沛林",
        "分級": "C",
        "上輪名次": 19,
        "異動": "down",
        "正式權重分數": 91.85,
        "實收": 0,
        "續單金額": 0,
        "總業績": 14950,
        "追續客單價": 0,
        "追續成交總數": 0,
        "追續金額": 0,
        "全部總業績": 14950,
        "追續單數": 0,
        "建議": "先把追續單與實收補起來，排名才有上升空間。"
      },
      {
        "名次": 23,
        "姓名": "蘇淑玲",
        "分級": "C",
        "上輪名次": 23,
        "異動": "flat",
        "正式權重分數": 0,
        "實收": 0,
        "續單金額": 0,
        "總業績": 0,
        "追續客單價": 0,
        "追續成交總數": 0,
        "追續金額": 0,
        "全部總業績": 0,
        "追續單數": 0,
        "建議": "今天先求破零，有分數才有派單空間。"
      },
      {
        "名次": 24,
        "姓名": "鄭上官",
        "分級": "C",
        "上輪名次": 24,
        "異動": "flat",
        "正式權重分數": 0,
        "實收": 0,
        "續單金額": 0,
        "總業績": 0,
        "追續客單價": 0,
        "追續成交總數": 0,
        "追續金額": 0,
        "全部總業績": 0,
        "追續單數": 0,
        "建議": "先解除空白狀態，後續才有排名意義。"
      }
    ],
    "分級": {
      "A1": [
        "馬秋香",
        "湯玉琦",
        "林沛昕",
        "廖姿惠"
      ],
      "A2": [
        "王珍珠",
        "林宜靜",
        "高美雲",
        "周美蓁",
        "許喬恩",
        "莉莉（新人）",
        "徐華妤",
        "高如郁"
      ],
      "B": [
        "李玲玲",
        "王梅慧",
        "梁依萍",
        "林佩君",
        "江麗勉",
        "陳百玲（新人）",
        "鄭珮恩",
        "謝啟芳"
      ],
      "C": [
        "陳玲華",
        "江沛林",
        "蘇淑玲",
        "鄭上官"
      ]
    },
    "每人一句建議": [
      {
        "name": "馬秋香",
        "rank": 1,
        "group": "A1",
        "text": "你這輪靠追續金額、總業績與客單價全面拉高，今天重點是把實收補上。"
      },
      {
        "name": "湯玉琦",
        "rank": 2,
        "group": "A1",
        "text": "你實收仍是全場第一，今天只要再補追續量就能穩住前段。"
      },
      {
        "name": "林沛昕",
        "rank": 3,
        "group": "A1",
        "text": "你追續單數與實收都有撐住，今天是繼續往前推的關鍵。"
      },
      {
        "name": "廖姿惠",
        "rank": 4,
        "group": "A1",
        "text": "你這輪客單價與追續金額明顯拉升，今天要把實收接起來。"
      },
      {
        "name": "王珍珠",
        "rank": 5,
        "group": "A2",
        "text": "你單數有量、總業績有底，今天差的是把金額與實收再拉高。"
      },
      {
        "name": "林宜靜",
        "rank": 6,
        "group": "A2",
        "text": "你有實收支撐，今天補上追續成交就能再往前。"
      },
      {
        "name": "高美雲",
        "rank": 7,
        "group": "A2",
        "text": "你這輪名次明顯上升，今天要把成交穩定度延續。"
      },
      {
        "name": "周美蓁",
        "rank": 8,
        "group": "A2",
        "text": "你有乾淨實收，今天只要再補一筆就能再動名次。"
      },
      {
        "name": "許喬恩",
        "rank": 9,
        "group": "A2",
        "text": "你跟周美蓁分數相同，今天補單就能拉開差距。"
      },
      {
        "name": "莉莉（新人）",
        "rank": 10,
        "group": "A2",
        "text": "你有實收亮點，今天先求穩定再往前推。"
      },
      {
        "name": "徐華妤",
        "rank": 11,
        "group": "A2",
        "text": "你客單價很漂亮，今天關鍵是把實收補起來。"
      },
      {
        "name": "高如郁",
        "rank": 12,
        "group": "A2",
        "text": "你有實收基礎，今天再補追續金額就能上推。"
      },
      {
        "name": "李玲玲",
        "rank": 13,
        "group": "B",
        "text": "你有單數但分數偏散，今天要提高追續金額。"
      },
      {
        "name": "王梅慧",
        "rank": 14,
        "group": "B",
        "text": "你單數有基本盤，今天差的是更高客單與實收。"
      },
      {
        "name": "梁依萍",
        "rank": 15,
        "group": "B",
        "text": "你有高客單切入點，今天先把下一筆成交接起來。"
      },
      {
        "name": "林佩君",
        "rank": 16,
        "group": "B",
        "text": "你這輪有有效分數，今天要把單數補上。"
      },
      {
        "name": "江麗勉",
        "rank": 17,
        "group": "B",
        "text": "你有追續成交，今天再補一筆就能拉高排序。"
      },
      {
        "name": "陳百玲（新人）",
        "rank": 18,
        "group": "B",
        "text": "你有累積，不急著衝，先把成交穩定做出來。"
      },
      {
        "name": "鄭珮恩",
        "rank": 19,
        "group": "B",
        "text": "你分數差距不大，今天先把追續金額補強。"
      },
      {
        "name": "謝啟芳",
        "rank": 20,
        "group": "B",
        "text": "你有成交但分數偏低，今天要先提高客單。"
      },
      {
        "name": "陳玲華",
        "rank": 21,
        "group": "C",
        "text": "你有總業績但缺追續與實收，今天先求有效成交。"
      },
      {
        "name": "江沛林",
        "rank": 22,
        "group": "C",
        "text": "先把追續單與實收補起來，排名才有上升空間。"
      },
      {
        "name": "蘇淑玲",
        "rank": 23,
        "group": "C",
        "text": "今天先求破零，有分數才有派單空間。"
      },
      {
        "name": "鄭上官",
        "rank": 24,
        "group": "C",
        "text": "先解除空白狀態，後續才有排名意義。"
      }
    ],
    "群組超精簡版": "📣【AI 派單公告｜5/4 結算 → 5/5 正式派單順序｜AI 比例原則版】\n\n一、審計結論\n\n審計結果：PASS\n本輪已依鎖死規則執行：先審計，後運算，後排序，再派單。\n\n三立奕心總表核對通過\n追續單成交：45 ✅\n全部總業績：717,358 ✅\n追續單金額：575,440 ✅\n實收總金額：60,460 ✅\n\n民視產品總表核對通過\n追續單成交：12 ✅\n全部總業績：490,000 ✅\n追續單金額：80,590 ✅\n實收總金額：18,160 ✅\n\n公司產品總表核對通過\n追續單成交：4 ✅\n全部總業績：45,710 ✅\n追續單金額：43,230 ✅\n實收總金額：35,250 ✅\n\n異常與提醒\n本輪三平台總表與個別明細加總一致。\n無漏算、無多算、無總盤衝突。\n\n二、整合總盤\n\n【追續單成交】61\n【全部總業績】1,253,068\n【追續單金額】699,260\n【實收總金額】113,870\n\n三、AI 比例原則\n\n本輪權重：\n實收總業績：3000\n追續單金額：2500\n全部總金額：1500\n追續客單價：1500\n追續單數量：1500\n\n正式權重分數＝\n實收比例分數＋追續金額比例分數＋全部總業績比例分數＋追續客單價比例分數＋追續單數量比例分數\n\n本輪全員最高值：\n實收總金額最高：30,480（湯玉琦）\n追續單金額最高：189,510（馬秋香）\n全部總業績最高：244,150（馬秋香）\n追續客單價最高：27,072.86（馬秋香）\n追續單數量最高：7（王珍珠、馬秋香、林沛昕）\n\n四、正式名次\n\n1、馬秋香｜【正式權重分數】7000.00｜【實收】0｜【追續金額】189,510｜【全部總業績】244,150｜【追續客單價】27,072.86｜【追續單數】7\n2、湯玉琦｜【正式權重分數】6253.76｜【實收】30,480｜【追續金額】62,860｜【全部總業績】106,860｜【追續客單價】12,572.00｜【追續單數】5\n3、林沛昕｜【正式權重分數】6073.93｜【實收】18,100｜【追續金額】95,580｜【全部總業績】126,148｜【追續客單價】13,654.29｜【追續單數】7\n4、廖姿惠｜【正式權重分數】3661.65｜【實收】2,980｜【追續金額】70,660｜【全部總業績】97,710｜【追續客單價】17,665.00｜【追續單數】4\n5、王珍珠｜【正式權重分數】3188.03｜【實收】0｜【追續金額】42,650｜【全部總業績】128,230｜【追續客單價】6,092.86｜【追續單數】7\n6、林宜靜｜【正式權重分數】3077.17｜【實收】7,600｜【追續金額】31,380｜【全部總業績】112,760｜【追續客單價】10,460.00｜【追續單數】3\n7、高美雲｜【正式權重分數】2930.50｜【實收】11,250｜【追續金額】26,070｜【全部總業績】42,490｜【追續客單價】6,517.50｜【追續單數】4\n8、周美蓁｜【正式權重分數】2292.29｜【實收】12,000｜【追續金額】12,000｜【全部總業績】12,000｜【追續客單價】12,000.00｜【追續單數】1\n9、許喬恩｜【正式權重分數】2292.29｜【實收】12,000｜【追續金額】12,000｜【全部總業績】12,000｜【追續客單價】12,000.00｜【追續單數】1\n10、莉莉（新人）｜【正式權重分數】2271.51｜【實收】11,880｜【追續金額】11,880｜【全部總業績】11,880｜【追續客單價】11,880.00｜【追續單數】1\n11、徐華妤｜【正式權重分數】2105.03｜【實收】0｜【追續金額】35,640｜【全部總業績】35,640｜【追續客單價】17,820.00｜【追續單數】2\n12、高如郁｜【正式權重分數】2039.76｜【實收】7,580｜【追續金額】15,500｜【全部總業績】37,640｜【追續客單價】7,750.00｜【追續單數】2\n13、李玲玲｜【正式權重分數】1842.52｜【實收】0｜【追續金額】18,560｜【全部總業績】78,690｜【追續客單價】4,640.00｜【追續單數】4\n14、王梅慧｜【正式權重分數】1742.81｜【實收】0｜【追續金額】20,800｜【全部總業績】52,600｜【追續客單價】5,200.00｜【追續單數】4\n15、梁依萍｜【正式權重分數】1486.55｜【實收】0｜【追續金額】14,280｜【全部總業績】47,640｜【追續客單價】14,280.00｜【追續單數】1\n16、林佩君｜【正式權重分數】1312.99｜【實收】0｜【追續金額】14,700｜【全部總業績】14,700｜【追續客單價】14,700.00｜【追續單數】1\n17、江麗勉｜【正式權重分數】898.95｜【實收】0｜【追續金額】9,480｜【全部總業績】13,460｜【追續客單價】4,740.00｜【追續單數】2\n18、陳百玲（新人）｜【正式權重分數】756.90｜【實收】0｜【追續金額】6,980｜【全部總業績】6,980｜【追續客單價】3,490.00｜【追續單數】2\n19、鄭珮恩｜【正式權重分數】752.80｜【實收】0｜【追續金額】5,750｜【全部總業績】14,500｜【追續客單價】2,875.00｜【追續單數】2\n20、謝啟芳｜【正式權重分數】518.85｜【實收】0｜【追續金額】2,980｜【全部總業績】16,300｜【追續客單價】2,980.00｜【追續單數】1\n21、陳玲華｜【正式權重分數】158.14｜【實收】0｜【追續金額】0｜【全部總業績】25,740｜【追續客單價】0｜【追續單數】0\n22、江沛林｜【正式權重分數】91.85｜【實收】0｜【追續金額】0｜【全部總業績】14,950｜【追續客單價】0｜【追續單數】0\n23、蘇淑玲｜【正式權重分數】0｜【實收】0｜【追續金額】0｜【全部總業績】0｜【追續客單價】0｜【追續單數】0\n24、鄭上官｜【正式權重分數】0｜【實收】0｜【追續金額】0｜【全部總業績】0｜【追續客單價】0｜【追續單數】0\n\n五、名次異動\n\n以上一輪 5/3 正式派單名次對照，本輪異動如下：\n\n上升\n馬秋香：2 → 1 ↑\n林沛昕：4 → 3 ↑\n廖姿惠：9 → 4 ↑\n高美雲：13 → 7 ↑\n徐華妤：21 → 11 ↑\n梁依萍：18 → 15 ↑\n林佩君：22 → 16 ↑\n\n下降\n湯玉琦：1 → 2 ↓\n林宜靜：3 → 6 ↓\n周美蓁：6 → 8 ↓\n許喬恩：7 → 9 ↓\n莉莉（新人）：8 → 10 ↓\n高如郁：10 → 12 ↓\n李玲玲：11 → 13 ↓\n王梅慧：12 → 14 ↓\n江麗勉：14 → 17 ↓\n陳百玲（新人）：17 → 18 ↓\n鄭珮恩：16 → 19 ↓\n謝啟芳：15 → 20 ↓\n陳玲華：20 → 21 ↓\n江沛林：19 → 22 ↓\n\n持平\n王珍珠：5 → 5 ＝\n蘇淑玲：23 → 23 ＝\n鄭上官：24 → 24 ＝\n\n六、A1／A2／B／C 分級\n\n🔴 A1｜高優先主力\n馬秋香\n湯玉琦\n林沛昕\n廖姿惠\n\n🟠 A2｜次主力追進\n王珍珠\n林宜靜\n高美雲\n周美蓁\n許喬恩\n莉莉（新人）\n徐華妤\n高如郁\n\n🟡 B組｜一般量單\n李玲玲\n王梅慧\n梁依萍\n林佩君\n江麗勉\n陳百玲（新人）\n鄭珮恩\n謝啟芳\n\n🟢 C組｜補位／觀察\n陳玲華\n江沛林\n蘇淑玲\n鄭上官\n\n七、每人一句建議\n\n1、馬秋香：你這輪靠追續金額、總業績與客單價全面拉高，今天重點是把實收補上。\n2、湯玉琦：你實收仍是全場第一，今天只要再補追續量就能穩住前段。\n3、林沛昕：你追續單數與實收都有撐住，今天是繼續往前推的關鍵。\n4、廖姿惠：你這輪客單價與追續金額明顯拉升，今天要把實收接起來。\n5、王珍珠：你單數有量、總業績有底，今天差的是把金額與實收再拉高。\n6、林宜靜：你有實收支撐，今天補上追續成交就能再往前。\n7、高美雲：你這輪名次明顯上升，今天要把成交穩定度延續。\n8、周美蓁：你有乾淨實收，今天只要再補一筆就能再動名次。\n9、許喬恩：你跟周美蓁分數相同，今天補單就能拉開差距。\n10、莉莉（新人）：你有實收亮點，今天先求穩定再往前推。\n11、徐華妤：你客單價很漂亮，今天關鍵是把實收補起來。\n12、高如郁：你有實收基礎，今天再補追續金額就能上推。\n13、李玲玲：你有單數但分數偏散，今天要提高追續金額。\n14、王梅慧：你單數有基本盤，今天差的是更高客單與實收。\n15、梁依萍：你有高客單切入點，今天先把下一筆成交接起來。\n16、林佩君：你這輪有有效分數，今天要把單數補上。\n17、江麗勉：你有追續成交，今天再補一筆就能拉高排序。\n18、陳百玲（新人）：你有累積，不急著衝，先把成交穩定做出來。\n19、鄭珮恩：你分數差距不大，今天先把追續金額補強。\n20、謝啟芳：你有成交但分數偏低，今天要先提高客單。\n21、陳玲華：你有總業績但缺追續與實收，今天先求有效成交。\n22、江沛林：先把追續單與實收補起來，排名才有上升空間。\n23、蘇淑玲：今天先求破零，有分數才有派單空間。\n24、鄭上官：先解除空白狀態，後續才有排名意義。\n\n正式前10名：1馬秋香 2湯玉琦 3林沛昕 4廖姿惠 5王珍珠 6林宜靜 7高美雲 8周美蓁 9許喬恩 10莉莉（新人）。\nA1：馬秋香、湯玉琦、林沛昕、廖姿惠。\nA2：王珍珠、林宜靜、高美雲、周美蓁、許喬恩、莉莉（新人）、徐華妤、高如郁。\nB組：李玲玲、王梅慧、梁依萍、林佩君、江麗勉、陳百玲（新人）、鄭珮恩、謝啟芳。\nC組：陳玲華、江沛林、蘇淑玲、鄭上官。",
    "preserveRankingOrder": true,
    "officialLock": {
      "preserveRankingOrder": true,
      "key": "0504-0505"
    }
  }
};
